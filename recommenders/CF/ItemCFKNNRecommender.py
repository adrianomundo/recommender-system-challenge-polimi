...
class ItemCFKNNRecommender(object):
    ...
    def fit(self, urm_train, top_k=10, shrink=30.0, normalize=True, similarity="jaccard"):
        ...
        sps.save_npz("../tmp/itemCF_matrix.npz", self.W_sparse)

    def compute_score(self, user_id, urm_train=None, load_matrix=False):

        if load_matrix:
            # self.urm_train = urm_train
            self.W_sparse = sps.load_npz("../tmp/itemCF_matrix.npz")
            self.W_sparse.tocsr()
